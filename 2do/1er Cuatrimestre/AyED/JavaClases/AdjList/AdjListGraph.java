package JavaClases.AdjList;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;

import JavaClases.JavaInterfaces.Edge;
import JavaClases.JavaInterfaces.Graph;
import JavaClases.JavaInterfaces.Vertex;
...
public class AdjListGraph<T> implements Graph<T> {
	...
	public void generateRandomGraph(int numVertices, int numEdges, int maxWeight) {
        ...
        for (int i = 0; i < numVertices; i++) {
            createVertex((T) ("Vertex " + i));
        }

        // Generar aristas aleatorias
        for (int i = 0; i < numEdges; i++) {
            int originIdx = random.nextInt(numVertices);
            int destIdx = random.nextInt(numVertices);
            int weight = random.nextInt(maxWeight) + 1; // Se suma 1 para evitar pesos de 0

            Vertex<T> origin = getVertex(originIdx);
            Vertex<T> destination = getVertex(destIdx);

            if (origin != null && destination != null) {
                connect(origin, destination, weight);
            }
        }
    }

	/*
	Generación Aleatoria de Vértices y Aristas:

	*Se ha añadido el método generateRandomGraph que crea numVertices vértices con datos aleatorios y luego genera numEdges aristas aleatorias entre ellos.
	*Los pesos de las aristas se generan aleatoriamente entre 1 y maxWeight.
	 */
}
